...
function buildPage(uid, snapshot) {
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"/><title>UID ${safeText(uid)} 纪念页</title><meta name="viewport" content="width=device-width,initial-scale=1"/>
  <style>body{font-family:system-ui;background:#f4f6fb;padding:20px}.card{max-width:900px;margin:0 auto;background:#fff;border:1px solid #dce4f2;border-radius:12px;padding:16px}table{width:100%;border-collapse:collapse}td,th{border-bottom:1px solid #ecf1f8;padding:8px;text-align:left}#video-list{padding-left:18px}#video-status.error{color:#b42318}code{background:#f5f7fb;padding:2px 4px;border-radius:4px}</style>
  </head><body data-uid="${safeText(uid)}"><div class="card"><h1>UID ${safeText(uid)} 纪念页</h1><p>页面已生成于 ${new Date(snapshot.createdAt).toLocaleString()}</p>
  <h2>上传数据</h2><table><thead><tr><th>文件名</th><th>大小</th><th>MIME</th></tr></thead><tbody>${rows}</tbody></table>
  <h2>视频概览</h2><p id="video-status">加载中...</p><ul id="video-list"></ul></div>
  <script>
  (function () {
    const MAX_DIRECT_RETRY = 3;
    const RETRY_DELAY_MS = 500;
    const DIRECT_API_BASE = 'https://uapis.cn/api/v1/social/bilibili';
    const uid = document.body.getAttribute('data-uid') || '';
    const statusEl = document.getElementById('video-status');
    const listEl = document.getElementById('video-list');

    if (!uid) {
      statusEl.textContent = 'uid 缺失';
    ...
    function buildDirectUrl(source, params) {
      const q = new URLSearchParams();
      if (source === 'allVid') {
        q.set('mid', params.uid);
        q.set('ps', '50');
        q.set('pn', String(params.pn || 1));
        return DIRECT_API_BASE + '/archives?' + q.toString();
      }
      if (source === 'comment') {
        q.set('bvid', params.bvid || '');
        q.set('pn', String(params.page || 1));
        return DIRECT_API_BASE + '/replies?' + q.toString();
      }
      if (source === 'vidInfo') {
        q.set('bvid', params.bvid || '');
        return DIRECT_API_BASE + '/view?' + q.toString();
      }
      throw new Error('invalid source');
    }

    function buildProxyUrl(source, params) {
      const q = new URLSearchParams();
      if (params.uid) q.set('uid', params.uid);
      if (params.bvid) q.set('bvid', params.bvid);
      if (params.pn) q.set('pn', String(params.pn));
      if (params.page) q.set('page', String(params.page));
      return '/api/proxy/' + encodeURIComponent(source) + '?' + q.toString();
    }
    ...
    async function fetchDirectThenProxy(source, params) {
      for (let attempt = 1; attempt <= MAX_DIRECT_RETRY; attempt += 1) {
        try {
          return await fetchJsonOnce(buildDirectUrl(source, params));
        } catch {
          if (attempt < MAX_DIRECT_RETRY) await sleep(RETRY_DELAY_MS * attempt);
        }
      }
      return fetchJsonOnce(buildProxyUrl(source, params));
    }
    ...
    function getPlayCount(video) {
      const value = Number(video?.play_count ?? video?.play ?? video?.stat?.view ?? 0);
      return Number.isFinite(value) ? value : 0;
    }

    async function loadTopVideos() {
      try {
        const data = await fetchDirectThenProxy('allVid', { uid, pn: 1 });
        const videos = normalizeVideos(data)
          .filter((v) => v && typeof v === 'object')
          .sort((a, b) => getPlayCount(b) - getPlayCount(a))
          .slice(0, 10);
        if (videos.length === 0) {
          statusEl.textContent = '暂无视频数据';
          return;
        }
        statusEl.textContent = '已加载播放量前 ' + videos.length + ' 条视频（前端直连失败 3 次后自动回退 workers 代理）。';
        listEl.innerHTML = videos.map((video) => {
          const title = escapeHtml(video.title || video.bvid || '未命名视频');
          const bvid = escapeHtml(video.bvid || '');
          const playCount = getPlayCount(video).toLocaleString();
          return '<li><strong>' + title + '</strong>' + (bvid ? ' <code>' + bvid + '</code>' : '') + ' · 播放 ' + playCount + '</li>';
        }).join('');
      } catch (err) {
        statusEl.textContent = '加载失败：' + String(err?.message || err);
        statusEl.className = 'error';
      }
    }

    // Expose helper for future frontend calls (comment/vidInfo can reuse same strategy).
    window.rememberProxyClient = { fetchDirectThenProxy };
    loadTopVideos();
  })();
  </script></body></html>`;
}
